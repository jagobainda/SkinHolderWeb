...
export const ProgressCard: React.FC<ProgressCardProps> = ({ platform, progress, total, totalPrice, onShowDetails, isEnabled }) => {
    ...
    return (
        <div className="group relative h-full">
            ...
            <div className="relative bg-surface-card rounded-xl p-6 shadow-lg border border-white/8 h-full flex flex-col justify-between">
                ...
                <div className="w-full bg-gray-700 rounded-full h-6 overflow-hidden">
                    <div className={`${config.bgColor} h-full transition-all duration-500 ease-out rounded-full`} style={{ width: `${percentage}%` }} />
                </div>
            </div>
        </div>
    );
